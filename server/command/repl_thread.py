...
class PythonREPLThread(threading.Thread):
    ...
    def response_to_client(self, code, data):
        ...
        if data:
            ...
            msg = {
                'type': 'response',
                'id': self.cmd_id,  # Use the REPL session ID
                'code': code,
                'data': data
            }
            ...
            while self.alive and self.p.poll() is None:
                ...
                try:
                    while True:
                            ...
                            if output:
                                ...
                                for i, out_line in enumerate(lines):
                                    if out_line or i < len(lines) - 1:  # Send non-empty lines or newlines
                                        self.response_to_client(0, {'stdout': out_line + ('\n' if i < len(lines) - 1 else '')})
                        ...
                        elif '\n' in self.output_buffer and not (self.output_buffer.endswith('>>> ') or self.output_buffer.endswith('... ')):
