        ...
        if acquired:
            with self._cleanup_lock:
                ...
                if user_file_key in self._active_executions:
                    ...
                    print(f"[EXEC-LOCK] Found existing execution for {user_file_key}: cmd_id={old_cmd_id}, replacing with {cmd_id}")
        ...
        with self._cleanup_lock:
            if user_file_key in self._active_executions:
                ...
                if active_cmd_id == cmd_id:
                    ...
                    print(f"[EXEC-LOCK] WARNING: cmd_id mismatch for {user_file_key}: active={active_cmd_id}, releasing={cmd_id}")
