    ...
    def get_file(self, username, role, data):
        ...
        try:
            ...
            if is_binary:
                ...
                return {"success": True, "content": content, "binary": True, "mime_type": mime_type}
            else:
    ...
    def list_directory(self, username, role, data):
            ...
            if local_path.exists():
                try:
                    ...
                    for item in all_items:
                        if item.is_dir():
                            subdirs.append({"name": item.name, "path": f"Local/{item.name}"})

                    logger.info(f"Returning {len(subdirs)} directories for professor")
                    return {"success": True, "directories": sorted(subdirs, key=lambda x: x["name"]), "files": []}
                except Exception as e:
        ...
        try:
            ...
            for item in full_path.iterdir():
                if item.is_dir():
                    directories.append({"name": item.name, "path": str(Path(dir_path) / item.name)})
                else:
                    files.append(
                        {
                            "name": item.name,
