        ...
        for user in users:
            ...
            try:
                ...
                db_manager.execute_query(
                    "DELETE FROM sessions WHERE user_id IN (SELECT id FROM users WHERE username = %s)", (username,)
                )
                db_manager.execute_query(
                ...
                db_manager.execute_query(
                    "DELETE FROM files WHERE user_id IN (SELECT id FROM users WHERE username = %s)", (username,)
                )
                db_manager.execute_query(
                    "DELETE FROM submissions WHERE user_id IN (SELECT id FROM users WHERE username = %s)", (username,)
                )
...
def load_class_from_csv(csv_path):
    ...
    with open(csv_path, "r", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            ...
            users.append({"username": username, "full_name": full_name, "role": role, "email": f"{username}@nyu.edu"})
...
def create_full_class_with_consistent_passwords(environment="production"):
    ...
    csv_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "adminData", "class_credentials_corrected_20250909_154857.csv"
    )
    ...
    for user_data in users_data:
        ...
        success, msg = manager.create_user(
            username=username, email=email, password=password, full_name=full_name, role=role
        )
        ...
        if success:
            ...
            failed_users.append({"username": username, "full_name": full_name, "error": msg, "role": role})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-remove", action="store_true", help="Do not remove existing users first")
    parser.add_argument("--verify-only", action="store_true", help="Only verify directories, do not create users")
    parser.add_argument("--test-consistency", action="store_true", help="Test password consistency across environments")
