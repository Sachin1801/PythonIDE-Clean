...
"""
                ...
                if self._write_to_python(safe_script):
                    ...
                    if callback:
                        callback({"cmd_id": cmd_id, "status": "completed", "output": output, "type": "script_result"})
                else:
                    if callback:
                        callback(
                            {
        ...
        def repl_thread():
            try:
                ...
                if self._write_to_python(command):
                    ...
                    if callback:
                        callback({"cmd_id": cmd_id, "status": "completed", "output": output, "type": "repl_result"})
                else:
                    if callback:
                        callback(
                            {"cmd_id": cmd_id, "status": "error", "output": "REPL not available", "type": "repl_result"}
                        )
                ...
                if callback:
                    callback({"cmd_id": cmd_id, "status": "error", "output": f"REPL error: {e}", "type": "repl_result"})
            finally:
    ...
    def get_system_stats(self) -> Dict:
        ...
        with self.session_lock:
            ...
            return {
                ...
                "limits": {"max_users": self.max_concurrent_users, "max_memory_mb": self.max_memory_total_mb},
                "system": {
            ...
            while True:
                try:
                    ...
                    if stats["total_memory_mb"] > self.max_memory_total_mb * 0.8:
                        logger.warning(
                            f"High memory usage: {stats['total_memory_mb']:.1f}MB/{self.max_memory_total_mb}MB"
                        )
