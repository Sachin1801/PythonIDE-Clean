#!/usr/bin/env python3
"""
Create user accounts from the class roster PDF
- Admin accounts: sl7927, sa9082, et2434 (professor rights)
- Student accounts: all others
- Export CSV with credentials
"""
import sys
import os
import csv
from datetime import datetime

sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from auth.user_manager_postgres import UserManager

def create_class_users():
    """Create all user accounts from the class roster"""
    manager = UserManager()
    
    # Class roster data extracted from PDF
    users_data = [
        # Admin accounts (professor rights)
        ('sl7927', 'Susan Liao', 'sl7927@nyu.edu', 'professor'),
        ('sa9082', 'Sachin Adlakha', 'sa9082@nyu.edu', 'professor'),
        ('et2434', 'Ethan Tan', 'et2434@nyu.edu', 'professor'),
        
        # Student accounts
        ('sa8820', 'Syed Ahnaf Ul Ahsan', 'sa8820@nyu.edu', 'student'),
        ('na3649', 'Nicole Akmetov', 'na3649@nyu.edu', 'student'),
        ('ntb5594', 'Nabi Burns-Min', 'ntb5594@nyu.edu', 'student'),
        ('hrb9324', 'Harry Byala', 'hrb9324@nyu.edu', 'student'),
        ('nd2560', 'Nikita Drovin-skiy', 'nd2560@nyu.edu', 'student'),
        ('ag11389', 'Adrian Garcia', 'ag11389@nyu.edu', 'student'),
        ('arg9667', 'Aarav Gupta', 'arg9667@nyu.edu', 'student'),
        ('lh4052', 'Liisa Hambazaza', 'lh4052@nyu.edu', 'student'),
        ('jh9963', 'Justin Hu', 'jh9963@nyu.edu', 'student'),
        ('ch5315', 'Rami Hu', 'ch5315@nyu.edu', 'student'),
        ('wh2717', 'Weijie Huang', 'wh2717@nyu.edu', 'student'),
        ('bsj5539', 'Maybelina J', 'bsj5539@nyu.edu', 'student'),
        ('fk2248', 'Falisha Khan', 'fk2248@nyu.edu', 'student'),
        ('nvk9963', 'Neil Khandelwal', 'nvk9963@nyu.edu', 'student'),
        ('sil9056', 'Simon Levine', 'sil9056@nyu.edu', 'student'),
        ('hl6459', 'Haoru Li', 'hl6459@nyu.edu', 'student'),
        ('zl3894', 'Jenny Li', 'zl3894@nyu.edu', 'student'),
        ('jom2045', 'Janell Magante', 'jom2045@nyu.edu', 'student'),
        ('arm9283', 'Amelia Mappus', 'arm9283@nyu.edu', 'student'),
        ('zm2525', 'Zhou Meng', 'zm2525@nyu.edu', 'student'),
        ('im2420', 'Ishaan Mukherjee', 'im2420@nyu.edu', 'student'),
        ('jn3143', 'Janvi Nagpal', 'jn3143@nyu.edu', 'student'),
        ('jan9106', 'Jacob Nathan', 'jan9106@nyu.edu', 'student'),
        ('djp10030', 'Darius Partovi', 'djp10030@nyu.edu', 'student'),
        ('ap10062', 'Alexandar Pelletier', 'ap10062@nyu.edu', 'student'),
        ('bap9618', 'Benjamin Piquet', 'bap9618@nyu.edu', 'student'),
        ('fp2331', 'Federico Pirelli', 'fp2331@nyu.edu', 'student'),
        ('srp8204', 'Shaina Pollak', 'srp8204@nyu.edu', 'student'),
        ('agr8457', 'Alex Reber', 'agr8457@nyu.edu', 'student'),
        ('shs9941', 'Suzie Sanford', 'shs9941@nyu.edu', 'student'),
        ('as19217', 'Albert Sun', 'as19217@nyu.edu', 'student'),
        ('mat9481', 'Mario Toscano', 'mat9481@nyu.edu', 'student'),
        ('cw4715', 'Chun-Hsiang Wang', 'cw4715@nyu.edu', 'student'),
        ('jw9248', 'Jingyuan Wang', 'jw9248@nyu.edu', 'student'),
        ('sz4766', 'Shengbo Zhang', 'sz4766@nyu.edu', 'student'),
    ]
    
    created_users = []
    failed_users = []
    
    print("Creating user accounts...")
    print("=" * 60)
    
    for username, full_name, email, role in users_data:
        # Set password based on role
        if role == 'professor':
            password = f"Admin@{username}"
        else:
            password = f"student@{username}"
        
        success, msg = manager.create_user(
            username=username,
            email=email,
            password=password,
            full_name=full_name,
            role=role
        )
        
        if success:
            created_users.append({
                'username': username,
                'full_name': full_name,
                'email': email,
                'password': password,
                'role': role
            })
            role_label = "ADMIN" if role == 'professor' else "STUDENT"
            print(f"✓ Created {role_label}: {username} ({full_name})")
        else:
            failed_users.append({
                'username': username,
                'full_name': full_name,
                'error': msg
            })
            print(f"✗ Failed to create {username}: {msg}")
    
    print("\n" + "=" * 60)
    print(f"Successfully created: {len(created_users)} accounts")
    print(f"Failed: {len(failed_users)} accounts")
    
    if failed_users:
        print("\nFailed accounts:")
        for user in failed_users:
            print(f"  - {user['username']} ({user['full_name']}): {user['error']}")
    
    # Generate CSV file with credentials
    csv_filename = f"class_credentials_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    csv_path = os.path.join(os.path.dirname(__file__), '..', '..', 'adminData', csv_filename)
    
    # Ensure adminData directory exists
    os.makedirs(os.path.dirname(csv_path), exist_ok=True)
    
    with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['Username', 'Full Name', 'Email', 'Password', 'Role', 'Notes']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        
        writer.writeheader()
        for user in created_users:
            role_note = "Admin Rights - Full Access" if user['role'] == 'professor' else "Student - Local Directory Access Only"
            writer.writerow({
                'Username': user['username'],
                'Full Name': user['full_name'],
                'Email': user['email'],
                'Password': user['password'],
                'Role': user['role'].title(),
                'Notes': role_note
            })
    
    print(f"\n✓ Credentials exported to: {csv_path}")
    print("\nPassword Format:")
    print("  - Admin accounts: Admin@{username}")
    print("  - Student accounts: student@{username}")
    print("\n⚠️  IMPORTANT: Share this CSV securely with the class!")
    
    return len(created_users), len(failed_users)

def reset_database():
    """Remove existing demo accounts before creating class accounts"""
    manager = UserManager()
    
    # Demo usernames to remove
    demo_users = [
        'professor', 'sa9082', 'jd1234', 'ab5678', 'mk9012', 'rs3456',
        'lj7890', 'tm2345', 'sw6789', 'dg0123', 'jm4567'
    ]
    
    print("Removing existing demo accounts...")
    for username in demo_users:
        try:
            # Use raw SQL to delete user and related sessions
            if manager.db.is_postgres:
                manager.db.execute_query("DELETE FROM sessions WHERE user_id IN (SELECT id FROM users WHERE username = %s)", (username,))
                manager.db.execute_query("DELETE FROM users WHERE username = %s", (username,))
            else:
                manager.db.execute_query("DELETE FROM sessions WHERE user_id IN (SELECT id FROM users WHERE username = ?)", (username,))
                manager.db.execute_query("DELETE FROM users WHERE username = ?", (username,))
            print(f"✓ Removed demo user: {username}")
        except Exception as e:
            print(f"✗ Could not remove {username}: {e}")

if __name__ == '__main__':
    import argparse
    
    parser = argparse.ArgumentParser(description='Create class user accounts from roster')
    parser.add_argument('--reset', action='store_true', 
                        help='Remove existing demo accounts first')
    parser.add_argument('--csv-only', action='store_true',
                        help='Only generate CSV from existing accounts (no account creation)')
    
    args = parser.parse_args()
    
    if args.reset:
        reset_database()
        print()
    
    if not args.csv_only:
        created, failed = create_class_users()
        
        if failed > 0:
            print(f"\n⚠️  {failed} accounts failed to create. Check the output above.")
            sys.exit(1)
        else:
            print(f"\n✅ All {created} accounts created successfully!")
    else:
        print("CSV-only mode: generating credentials file from existing accounts...")
        # This would require querying existing accounts - implement if needed