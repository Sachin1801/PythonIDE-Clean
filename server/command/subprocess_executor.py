...
class SubprocessExecutor:
    ...
    def execute_async(self, code: str, callback=None, **kwargs) -> str:
        ...
        request = {"id": exec_id, "code": code, "callback": callback, **kwargs}

        self.execution_queue.put(request)
        return exec_id

    def execute_sync(self, code: str, timeout: int = None, memory_limit: int = None) -> Dict[str, Any]:
    ...
    def _run_code(self, code: str, exec_id: str, timeout: int, memory_limit: int) -> Dict[str, Any]:
...
class ExecutionManager:
    ...
    def execute_for_user(self, username: str, code: str, callback=None, **kwargs) -> Dict[str, Any]:
        ...
        if user_count >= 2:  # Max 2 concurrent per user
            return {"error": "Rate limit exceeded. Please wait for current executions to complete.", "queued": False}

        # Check total concurrent limit
        if self.executor.get_active_count() >= self.max_concurrent:
            return {"error": "Server at capacity. Please try again in a moment.", "queued": False}
        ...
        return {"exec_id": exec_id, "queued": True, "message": "Code queued for execution"}
...
if __name__ == "__main__":
    ...
    for i in range(3):
        result = manager.execute_for_user(f"user{i}", f'print("User {i} code")', timeout=5)
