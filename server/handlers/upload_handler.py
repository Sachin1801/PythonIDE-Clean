    ...
    def post(self):
        ...
        try:
            ...
            if user_info["username"] not in admin_accounts:
                self.set_status(403)
                self.write(json.dumps({"success": False, "error": "Only admin users can upload files"}))
                return
            ...
            if not project_name or not filename:
                self.set_status(400)
                self.write(
                    json.dumps({"success": False, "error": "Missing required parameters: projectName, filename"})
                )
                return
            ...
            try:
                ...
                self.set_status(500)
                self.write(json.dumps({"success": False, "error": f"Failed to save file: {str(e)}"}))
                return
