...
class ResourceAnalyzer:
    ...
    def analyze_multiprocess_architecture(self, users: int) -> ResourceProfile:
        ...
        cpu_needed = (
            active_processes * self.PROCESS_CPU_OVERHEAD +
            active_processes * 0.15  # Assume 15% CPU per active process
        )
    ...
    def _get_recommendations(self, multiprocess: ResourceProfile, threading: ResourceProfile, hybrid: ResourceProfile) -> List[str]:
        ...
        if threading.memory_mb < multiprocess.memory_mb:
            ...
            recommendations.append(f"Threading saves {savings_mb:.1f}MB memory ({((savings_mb/multiprocess.memory_mb)*100):.1f}%)")
        ...
        if process_reduction > 0:
            recommendations.append(f"Threading reduces processes from {multiprocess.processes} to {threading.processes}")
    ...
    def capacity_analysis_for_current_hw(self) -> Dict:
        ...
        for users in [10, 15, 20, 25, 30, 38]:
            ...
            for arch_name, arch_data in comparison['architectures'].items():
                ...
                if not cpu_fits:
                    arch_data['bottleneck'].append(f"CPU: need {arch_data['cpu_cores_needed']:.2f}, have {current_limits['cpu_cores']}")
                if not memory_fits:
                    arch_data['bottleneck'].append(f"Memory: need {arch_data['memory_mb']:.1f}MB, have {current_limits['memory_mb']}MB")
